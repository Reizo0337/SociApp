...
export class ProjectController {
  ...
  create(@Body() createProjectDto: CreateProjectDto, @UploadedFiles() files: Express.Multer.File[]) {
    if (files && files.length > 0) {
      createProjectDto.pdfPath = files.map(file => `/uploads/projects/${file.filename}`);
    }
  ...
  update(
    ...
    if (files && files.length > 0) {
      updateProjectDto.pdfPath = files.map(file => `/uploads/projects/${file.filename}`);
    }
